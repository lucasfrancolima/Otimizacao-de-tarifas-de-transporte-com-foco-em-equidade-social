# -*- coding: utf-8 -*-
"""
Comparador de modelos (OPTIMA)
- Estima as especificações pedidas
- Reporta: LR para aninhados, AIC/BIC, log-likelihood final e taxa de acerto (hit rate)
"""
...
CHOICE_VAR = "Choice"  # nome da coluna de escolha (0 = TC, 1 = TI, 2 = modos suaves)
...
SPECS = [
    ...
    'TIME_COSTxRENDA_LH_GENERIC',
    'TIME_GENERIC_COSTxRENDA_LH_SPECIFIC',
    'TIME_COSTxRENDA_LMH_GENERIC',
    'TIME_GENERIC_COSTxRENDA_LMH_SPECIFIC',
    'TIME_GENERIC_COSTxRENDA_ALLFAIXAS_GENERIC',
    'TIME_GENERIC_COSTxRENDA_ALLFAIXAS_SPECIFIC',
]
...
# Filtros conforme dicionário: remove Choice=2 (modos suaves) e missing
mask = (
    (df[CHOICE_VAR] != -1) &
    (df[CHOICE_VAR] != 2) &
    (df['Income'] != -1) &
...
globals().update(database.variables)  # expõe Choice, TimePT, TimeCar, MarginalCostPT, CostCarCHF, INC_*, LI/MI/HI

# =========================
# HELPERS DE MODELAÇÃO
# =========================
...
def build_betas(spec: str):
    ...
    if spec == 'TIME_COST_GENERIC':
        betas['B_TIME'] = B('B_TIME'); betas['B_COST'] = B('B_COST')

    elif spec == 'TIME_COST_SPECIFIC':
        betas['B_TIME_PT'] = B('B_TIME_PT'); betas['B_TIME_CAR'] = B('B_TIME_CAR')
        betas['B_COST_PT'] = B('B_COST_PT'); betas['B_COST_CAR'] = B('B_COST_CAR')
        ...
        betas['B_TIME'] = B('B_TIME')
        betas['B_COST_LI'] = B('B_COST_LI'); betas['B_COST_HI'] = B('B_COST_HI')

    elif spec == 'TIME_GENERIC_COSTxRENDA_LH_SPECIFIC':
        betas['B_TIME'] = B('B_TIME')
        betas['B_COST_PT_LI'] = B('B_COST_PT_LI'); betas['B_COST_PT_HI'] = B('B_COST_PT_HI')
        betas['B_COST_CAR_LI'] = B('B_COST_CAR_LI'); betas['B_COST_CAR_HI'] = B('B_COST_CAR_HI')

    elif spec == 'TIME_COSTxRENDA_LMH_GENERIC':
        betas['B_TIME'] = B('B_TIME')
        betas['B_COST_LI'] = B('B_COST_LI'); betas['B_COST_MI'] = B('B_COST_MI'); betas['B_COST_HI'] = B('B_COST_HI')
...
def simulate_probs_and_hit(spec: str, betas_values, V_PT, V_CAR):
...
def compute_vot(results, spec: str):
    ...
    def safe_ratio(num, den):
        ...
        return -num/den
    if spec == 'TIME_COST_GENERIC' and {'B_TIME','B_COST'} <= betas.keys():
...
for spec in SPECS:
    try:
        ...
        hit   = simulate_probs_and_hit(spec, results.getBetaValues(), V_PT, V_CAR)
...
if summary.empty:
    ...
    if lr_rows:
        ...
        print("\n=== TESTES LR (modelos aninhados) ===")
        print(lr_df.to_string(index=False))
    else:
        print("\nSem pares aninhados válidos para LR.")

    # 3) Salva CSVs auxiliares
    out_dir = Path(__file__).parent
    tbl.to_csv(out_dir / f"{MODEL_PREFIX}__comparativo_info_hit.csv", index=False)
    if lr_rows:
